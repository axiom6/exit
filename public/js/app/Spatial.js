...
Spatial = (function() {
  ...
  class Spatial {
    ...
    constructor(stream, trip) {
      ...
      this.subscribe();
      Util.noop(this.pushLocations, this.pushNavLocations, this.pushGeoLocators, this.pushAddressForLatLon, this.mileSegs);
      Util.noop(this.mileLatLonSpherical, this.mileLatLon2);
    }
    ...
    onLocation(location) {
      return console.log('Spatial.onLocation', location);
    }
    ...
    pushNavLocations() {
      ...
      onError = (error) => {
        return console.error('Spatia.pushLocation()', ' Unable to get your location', error);
      };
      options = {
    ...
    pushGeoLocators() {
      var geolocator, onError, onSuccess, options;
      geolocator = new this.google['maps'].Geolocator(); // ???   
      if (Spatial.PushLocationsOn) {
        return;
      } else {
        Spatial.PushLocationsOn = true;
    ...
    pushAddressForLatLon(latLon) {
      var geocoder, geolocator, latlng, onReverseGeo;
      ...
      onReverseGeo = function(results, status) {
        if (status === this.google['maps']['GeocoderStatus']['OK']) {
          ...
          return console.error('Spatial.pushAddressForLatLon() bad status from @google.maps', status);
        }
      };
      latlng = new this.google['maps'].LatLng(latLon.lat, latLon.lon);
