...
DestinationUI = class DestinationUI {
  constructor(stream) {
    this.stream = stream;
    this.thresholdUC = new ThresholdUC(this.stream, 'Destin', [0, 60, 100, 40], [50, 20, 50, 80]);
    this.sources = Data.Destinations; // For now we access sources     from static data
    this.destins = Data.Destinations; // For now we access destins from static data
