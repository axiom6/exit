...
(function() {
  ...
  Test = (function() {
    ...
    Test.rad = function(deg) {
      return deg * Math.PI / 180;
    };

    Test.deg = function(rad) {
      return rad * 180 / Math.PI;
    };

    Test.sin = function(deg) {
      return Math.sin(Visual.rad(deg));
    };

    Test.cos = function(deg) {
      return Math.cos(Visual.rad(deg));
    };

    Test.svgDeg = function(deg) {
      return 360 - deg;
    };

    Test.svgRad = function(rad) {
      return 2 * Math.PI - rad;
    };

    Test.radSvg = function(deg) {
      return Visual.rad(360 - deg);
    };

    Test.degSvg = function(rad) {
      return Visual.deg(2 * Math.PI - rad);
    };

    Test.sinSvg = function(deg) {
      return Math.sin(Visual.radSvg(deg));
    };

    Test.cosSvg = function(deg) {
      return Math.cos(Visual.radSvg(deg));
    };

    Test.pad2 = function(n) {
      var s;
      s = n.toString();
      if (0 <= n && n <= 9) {
        s = '&nbsp;' + s;
    ...
    Test.quotes = function(str) {
      return '"' + str + '"';
    };

    Test.within = function(beg, deg, end) {
      return beg <= deg && deg <= end;
    };

    Test.isZero = function(v) {
      return -0.01 < v && v < 0.01;
    };

    Test.floor = function(x, dx) {
      var dr;
      dr = Math.round(dx);
      return Math.floor(x / dr) * dr;
    };

    Test.ceil = function(x, dx) {
      var dr;
      dr = Math.round(dx);
      return Math.ceil(x / dr) * dr;
    };

    Test.to = function(a, a1, a2, b1, b2) {
      return (a - a1) / (a2 - a1) * (b2 - b1) + b1;
    };

    Test.angle = function(x, y) {
      var ang;
    ...
    Test.angleSvg = function(x, y) {
      return Visual.angle(x, -y);
    };

    Test.minRgb = function(rgb) {
      return Math.min(rgb.r, rgb.g, rgb.b);
    };

    Test.maxRgb = function(rgb) {
      return Math.max(rgb.r, rgb.g, rgb.b);
    };

    Test.sumRgb = function(rgb) {
      return rgb.r + rgb.g + rgb.b;
    };

    Test.hexCss = function(hex) {
      return "#" + (hex.toString(16));
    };

    Test.rgbCss = function(rgb) {
      return "rgb(" + rgb.r + "," + rgb.g + "," + rgb.b + ")";
    };

    Test.hslCss = function(hsl) {
      return "hsl(" + hsl.h + "," + (hsl.s * 100) + "%," + (hsl.l * 100) + "%)";
    };

    Test.hsiCss = function(hsi) {
      return Visual.hslCss(Visual.rgbToHsl(Visual.hsiToRgb(hsi)));
    };

    Test.hsvCss = function(hsv) {
      return Visual.hslCss(Visual.rgbToHsl(Visual.hsvToRgb(hsv)));
    };

    Test.roundRgb = function(rgb, f) {
      if (f == null) {
        f = 1.0;
      }
      return {
    ...
    Test.rgbHex = function(rgb) {
      return rgb.r * 4096 + rgb.g * 256 + rgb.b;
    };

    Test.cssRgb = function(str) {
    ...
    Test.rgbsToHsl = function(red, green, blue) {
