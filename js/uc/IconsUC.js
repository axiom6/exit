  ...
  IconsUC = (function() {
    ...
    IconsUC.prototype.html = function() {
      ...
      for (i = 0, len = ref.length; i < len; i++) {
        spec = ref[i];
        htm += "<div id=\"" + (Util.id(spec.name, 'Icon', this.subject)) + "\" class=\"" + (Util.css(spec.css)) + "\"><i class=\"fa fa-" + spec.icon + "\"></i><div>" + spec.name + "</div></div>";
      }
      htm += "</div></div></div>";
      return htm;
    };
    ...
    IconsUC.prototype.events = function() {
      ...
      for (i = 0, len = ref.length; i < len; i++) {
        spec = ref[i];
        results.push(this.stream.event(this.subject, this.$find(spec.name), 'click', spec.name));
      }
      return results;
    };
